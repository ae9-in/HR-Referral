...
const DecayCard = ({ 
  ...
  useEffect(() => {
    ...
    const render = () => {
      let targetX = lerp(imgValues.imgTransforms.x, map(cursor.current.x, 0, winsize.current.width, -80, 80), 0.1);
      let targetY = lerp(imgValues.imgTransforms.y, map(cursor.current.y, 0, winsize.current.height, -80, 80), 0.1);
      let targetRz = lerp(imgValues.imgTransforms.rz, map(cursor.current.x, 0, winsize.current.width, -5, 5), 0.1);
      ...
      imgValues.displacementScale = lerp(
        ...
        map(cursorTravelledDistance, 0, 150, 0, 300),
        0.06
      );
