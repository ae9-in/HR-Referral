...
class ImageTrailVariant1 {
  ...
  constructor(container: HTMLElement) {
    ...
    this.images = Array.from(this.DOM.el.querySelectorAll('.content__img') as NodeListOf<HTMLElement>).map(img => new ImageItem(img));
...
const variantMap: any = { 1: ImageTrailVariant1 };
...
export default function ImageTrail({ items = [], variant = 1 }: { items?: string[], variant?: number }) {
  ...
  return (
    <div className="image-trail-content" ref={containerRef}>
      {items.map((url, i) => (
